...
const SignUp = ({ navigation }: AuthNavigationProps<"SignUp">): JSX.Element => {
  ...
  const handleNavigation = (email:string,password:string) => {
    navigation.navigate("Country",{email,password})
  }
  ...
  return (
    <Box alignItems="center" w="100%" mt={71}>
      <Box maxW={375} w="90%">
        ...
        <Formik initialValues={{ email: "", password: "", confirmPassword: "" }} validationSchema={signupSchema} onSubmit={({email,password}) => {
          handleNavigation(email,password)
        }}>
          {({ handleSubmit, handleChange, values, errors, touched }) => (
            <>
              ...
              <InputField input={values.confirmPassword} getInput={handleChange('confirmPassword')} label="Confirm Password" placeholder="Confirm Password" setVisibilty={setVisibilty} visibility={show} />
